#include <vector>

#include "gtest/gtest.h"

#include "caffe/blob.hpp"
#include "caffe/common.hpp"
#include "caffe/filler.hpp"
#include "caffe/layers/matrix_transpose_layer.hpp"

#include "caffe/test/test_caffe_main.hpp"
#include "caffe/test/test_gradient_check_util.hpp"

namespace caffe {

#ifndef CPU_ONLY
extern cudaDeviceProp CAFFE_TEST_CUDA_PROP;
#endif

template <typename TypeParam>
class MatrixTransposeLayerTest : public MultiDeviceTest<TypeParam> {
  typedef typename TypeParam::Dtype Dtype;
 protected:
  MatrixTransposeLayerTest()
      : blob_bottom_X_(new Blob<Dtype>()),
        blob_bottom_Y_(new Blob<Dtype>()),
        blob_bottom_x_(new Blob<Dtype>()),
        blob_top_(new Blob<Dtype>()) {
    // reshape
    vector<int> shape(3);
    shape[0]=2; shape[1]=5; shape[2]=3;
    blob_bottom_X_->Reshape(shape);
    vector<int> shape3(4);
    shape3[0]=5; shape3[1]=2; shape3[2]=3; shape3[3]=2;
    blob_bottom_Y_->Reshape(shape3);
    vector<int> shape2(2);
    shape2[0]=2; shape2[1]=3;
    blob_bottom_x_->Reshape(shape2);
    // fill the values
    FillerParameter filler_param;
    UniformFiller<Dtype> filler(filler_param);
    filler.Fill(this->blob_bottom_X_);
    filler.Fill(this->blob_bottom_x_);
    blob_top_vec_.push_back(blob_top_);
  }
  virtual ~MatrixTransposeLayerTest() {
    delete blob_bottom_X_;
    delete blob_bottom_Y_;
    delete blob_bottom_x_;
    delete blob_top_;
  }
  Blob<Dtype>* const blob_bottom_X_;
  Blob<Dtype>* const blob_bottom_Y_;
  Blob<Dtype>* const blob_bottom_x_;
  Blob<Dtype>* const blob_top_;
  vector<Blob<Dtype>*> blob_bottom_vec_;
  vector<Blob<Dtype>*> blob_top_vec_;
};

TYPED_TEST_CASE(MatrixTransposeLayerTest, TestDtypesAndDevices);

TYPED_TEST(MatrixTransposeLayerTest, TestSetUp) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  shared_ptr<MatrixTransposeLayer<Dtype> > layer(
      new MatrixTransposeLayer<Dtype>(layer_param));

  //X
  this->blob_bottom_vec_.clear();
  this->blob_bottom_vec_.push_back(this->blob_bottom_X_);
  layer->SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
  EXPECT_EQ(this->blob_top_->num_axes(), 3);
  EXPECT_EQ(this->blob_top_->shape(0), 2);
  EXPECT_EQ(this->blob_top_->shape(1), 3);
  EXPECT_EQ(this->blob_top_->shape(2), 5);
  
  //Y
  this->blob_bottom_vec_.clear();
  this->blob_bottom_vec_.push_back(this->blob_bottom_Y_);
  layer->SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
  EXPECT_EQ(this->blob_top_->num_axes(), 4);
  EXPECT_EQ(this->blob_top_->shape(0), 5);
  EXPECT_EQ(this->blob_top_->shape(1), 2);
  EXPECT_EQ(this->blob_top_->shape(2), 2);
  EXPECT_EQ(this->blob_top_->shape(3), 3);

  //x
  this->blob_bottom_vec_.clear();
  this->blob_bottom_vec_.push_back(this->blob_bottom_x_);
  layer->SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
  EXPECT_EQ(this->blob_top_->num_axes(), 2);
  EXPECT_EQ(this->blob_top_->shape(0), 3);
  EXPECT_EQ(this->blob_top_->shape(1), 2);
}

template <typename Dtype>
static void print_mat(
  const int M, const int N,
  const Dtype* Z, const char* name)
{
  std::cout<<name<<"("<<M<<"x"<<N<<"):"<<std::endl;
  for(int m=0; m<M; ++m) {
    for(int n=0; n<N; ++n) {
      std::cout<<Z[m*N+n]<<" ";
    }
    std::cout<<std::endl;
  }
}

template <typename Dtype>
static void check_mat_trans(
  const int M, const int N,
  const Dtype* X, const Dtype* Z)
{
#if 0
  print_mat<Dtype>(M,K,X,"X");
  print_mat<Dtype>(M,N,Z,"Z");
  std::cout<<"-----------"<<std::endl;
#endif
  for(int m=0; m<M; ++m) {
    for(int n=0; n<N; ++n) {
      const Dtype z=Z[m*N+n]; //Z <MxN>
      Dtype expected_z=X[n*M+m]; //X <NxM>
      EXPECT_FLOAT_EQ(expected_z, z);
    }
  }
}

TYPED_TEST(MatrixTransposeLayerTest, TestForwardX) {
  typedef typename TypeParam::Dtype Dtype;
  this->blob_bottom_vec_.clear();
  this->blob_bottom_vec_.push_back(this->blob_bottom_X_);
  bool IS_VALID_CUDA = false;
#ifndef CPU_ONLY
  IS_VALID_CUDA = CAFFE_TEST_CUDA_PROP.major >= 2;
#endif
  if (Caffe::mode() == Caffe::CPU ||
      sizeof(Dtype) == 4 || IS_VALID_CUDA) {
    LayerParameter layer_param;
    shared_ptr<MatrixTransposeLayer<Dtype> > layer(
        new MatrixTransposeLayer<Dtype>(layer_param));
    layer->SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
    layer->Forward(this->blob_bottom_vec_, this->blob_top_vec_);
    const Dtype* X_data = this->blob_bottom_X_->cpu_data();
    const Dtype* Z_data = this->blob_top_->cpu_data();
    const int B = this->blob_bottom_X_->shape(0);
    const int M = this->blob_top_->shape(1);
    const int N = this->blob_top_->shape(2);
    const int X_stride = this->blob_bottom_X_->count(1);
    const int Z_stride = this->blob_top_->count(1);
    for (int b = 0; b < B; ++b) {
      check_mat_trans<Dtype>(M,N,
        X_data+b*X_stride,
        Z_data+b*Z_stride);
    }
  } else {
    LOG(ERROR) << "Skipping test due to old architecture.";
  }
}

TYPED_TEST(MatrixTransposeLayerTest, TestForwardY) {
  typedef typename TypeParam::Dtype Dtype;
  this->blob_bottom_vec_.clear();
  this->blob_bottom_vec_.push_back(this->blob_bottom_Y_);
  bool IS_VALID_CUDA = false;
#ifndef CPU_ONLY
  IS_VALID_CUDA = CAFFE_TEST_CUDA_PROP.major >= 2;
#endif
  if (Caffe::mode() == Caffe::CPU ||
      sizeof(Dtype) == 4 || IS_VALID_CUDA) {
    LayerParameter layer_param;
    shared_ptr<MatrixTransposeLayer<Dtype> > layer(
        new MatrixTransposeLayer<Dtype>(layer_param));
    layer->SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
    layer->Forward(this->blob_bottom_vec_, this->blob_top_vec_);
    const Dtype* Y_data = this->blob_bottom_Y_->cpu_data();
    const Dtype* Z_data = this->blob_top_->cpu_data();
    const int B = this->blob_bottom_Y_->count(0,2);
    const int M = this->blob_top_->shape(2);
    const int N = this->blob_top_->shape(3);
    const int Y_stride = this->blob_bottom_Y_->count(2);
    const int Z_stride = this->blob_top_->count(2);
    for (int b = 0; b < B; ++b) {
      check_mat_trans<Dtype>(M,N,
        Y_data+b*Y_stride,
        Z_data+b*Z_stride);
    }
  } else {
    LOG(ERROR) << "Skipping test due to old architecture.";
  }
}

TYPED_TEST(MatrixTransposeLayerTest, TestForwardx) {
  typedef typename TypeParam::Dtype Dtype;
  this->blob_bottom_vec_.clear();
  this->blob_bottom_vec_.push_back(this->blob_bottom_x_);
  bool IS_VALID_CUDA = false;
#ifndef CPU_ONLY
  IS_VALID_CUDA = CAFFE_TEST_CUDA_PROP.major >= 2;
#endif
  if (Caffe::mode() == Caffe::CPU ||
      sizeof(Dtype) == 4 || IS_VALID_CUDA) {
    LayerParameter layer_param;
    shared_ptr<MatrixTransposeLayer<Dtype> > layer(
        new MatrixTransposeLayer<Dtype>(layer_param));
    layer->SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
    layer->Forward(this->blob_bottom_vec_, this->blob_top_vec_);
    const Dtype* x_data = this->blob_bottom_x_->cpu_data();
    const Dtype* z_data = this->blob_top_->cpu_data();
    const int M = this->blob_top_->shape(0);
    const int N = this->blob_top_->shape(1);
    check_mat_trans<Dtype>(M,N,
      x_data,
      z_data);
  } else {
    LOG(ERROR) << "Skipping test due to old architecture.";
  }
}

TYPED_TEST(MatrixTransposeLayerTest, TestGradientX) {
  typedef typename TypeParam::Dtype Dtype;
  this->blob_bottom_vec_.clear();
  this->blob_bottom_vec_.push_back(this->blob_bottom_X_);
  bool IS_VALID_CUDA = false;
#ifndef CPU_ONLY
  IS_VALID_CUDA = CAFFE_TEST_CUDA_PROP.major >= 2;
#endif
  if (Caffe::mode() == Caffe::CPU ||
      sizeof(Dtype) == 4 || IS_VALID_CUDA) {
    LayerParameter layer_param;
    MatrixTransposeLayer<Dtype> layer(layer_param);
    GradientChecker<Dtype> checker(1e-2, 1e-3);
    checker.CheckGradientExhaustive(&layer, this->blob_bottom_vec_,
        this->blob_top_vec_);
  } else {
    LOG(ERROR) << "Skipping test due to old architecture.";
  }
}

TYPED_TEST(MatrixTransposeLayerTest, TestGradientY) {
  typedef typename TypeParam::Dtype Dtype;
  this->blob_bottom_vec_.clear();
  this->blob_bottom_vec_.push_back(this->blob_bottom_Y_);
  bool IS_VALID_CUDA = false;
#ifndef CPU_ONLY
  IS_VALID_CUDA = CAFFE_TEST_CUDA_PROP.major >= 2;
#endif
  if (Caffe::mode() == Caffe::CPU ||
      sizeof(Dtype) == 4 || IS_VALID_CUDA) {
    LayerParameter layer_param;
    MatrixTransposeLayer<Dtype> layer(layer_param);
    GradientChecker<Dtype> checker(1e-2, 1e-3);
    checker.CheckGradientExhaustive(&layer, this->blob_bottom_vec_,
        this->blob_top_vec_);
  } else {
    LOG(ERROR) << "Skipping test due to old architecture.";
  }
}


TYPED_TEST(MatrixTransposeLayerTest, TestGradientx) {
  typedef typename TypeParam::Dtype Dtype;
  this->blob_bottom_vec_.clear();
  this->blob_bottom_vec_.push_back(this->blob_bottom_x_);
  bool IS_VALID_CUDA = false;
#ifndef CPU_ONLY
  IS_VALID_CUDA = CAFFE_TEST_CUDA_PROP.major >= 2;
#endif
  if (Caffe::mode() == Caffe::CPU ||
      sizeof(Dtype) == 4 || IS_VALID_CUDA) {
    LayerParameter layer_param;
    MatrixTransposeLayer<Dtype> layer(layer_param);
    GradientChecker<Dtype> checker(1e-2, 1e-3);
    checker.CheckGradientExhaustive(&layer, this->blob_bottom_vec_,
        this->blob_top_vec_);
  } else {
    LOG(ERROR) << "Skipping test due to old architecture.";
  }
}

}  // namespace caffe
